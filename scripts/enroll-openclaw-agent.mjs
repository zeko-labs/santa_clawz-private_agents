...
Notes:
  ...
  Without --serve, your existing PublicClawz ingress must serve the challenge file path.
`);
}
...
function buildAgentEnvFile(input) {
  const lines = [
    "# Generated by SantaClawz PublicClawz enrollment.",
...
async function sendHeartbeatOnce(config) {
  return requestJson(`${config.apiBase}/api/agents/${encodeURIComponent(config.agentId)}/heartbeat`, {
    ...
    body: JSON.stringify({
      ...
      status: "live",
      ttlSeconds: 30,
      note: "SantaClawz PublicClawz enrollment heartbeat."
    })
  });
}
