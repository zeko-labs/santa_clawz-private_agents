...
- `payment-state` is the canonical resume lookup for fixed-price and quote-intent payments. It can be queried by `ledgerId`, `intentId`, `requestId`, or `paymentPayloadDigestSha256` and returns `retryResume.safeToRetrySamePayload`, `nextAction`, and the best state/retry endpoint. Public buyer lookups are redacted and limited to recovery state; admin/API-key callers can use the full ledger diagnostics.
